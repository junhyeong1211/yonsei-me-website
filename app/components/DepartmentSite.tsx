...
function NoticeRow({ notice, locale }: { notice: Notice; locale: Locale }) {
  const category = notice.isPinned ? tx(locale, "고정", "Pinned") : tx(locale, notice.category, "Notice");

  return (
    <Link className="notice-row" href={hrefFor(locale, `/news/notices/${notice.slug}`)}>
      <span className={`notice-category ${notice.isPinned ? "important" : ""}`}>{category}</span>
...
function NoticeDirectory({ locale, searchParams }: { locale: Locale; searchParams: Record<string, string> }) {
  ...
  const results = [...notices]
    .filter((notice) => audience === "all" || notice.audience === audience)
    .filter((notice) => !query || `${t(notice.title, locale)} ${t(notice.author, locale)}`.toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => Number(b.isPinned) - Number(a.isPinned) || b.publishedAt.localeCompare(a.publishedAt));
  const pageSize = 10;
  const totalPages = Math.max(1, Math.ceil(results.length / pageSize));
  const visibleResults = results.slice((page - 1) * pageSize, page * pageSize);
  const selectAudience = (nextAudience: string) => { setAudience(nextAudience); setPage(1); };

  return <><PageHeader eyebrow="NOTICE" title={tx(locale, "공지사항", "Notices")} description={tx(locale, "학부와 대학원 주요 안내를 빠르게 확인하세요.", "Find undergraduate and graduate announcements.")} /><section className="section content-section"><div className="container"><div className="notice-toolbar"><div className="tabs" role="tablist" aria-label={tx(locale, "공지 구분", "Notice audience")}><button type="button" role="tab" aria-selected={audience === "all"} onClick={() => selectAudience("all")}>{tx(locale, "전체", "All")}</button><button type="button" role="tab" aria-selected={audience === "undergraduate"} onClick={() => selectAudience("undergraduate")}>{tx(locale, "학부공지", "Undergraduate")}</button><button type="button" role="tab" aria-selected={audience === "graduate"} onClick={() => selectAudience("graduate")}>{tx(locale, "대학원공지", "Graduate")}</button></div><label className="inline-search"><span className="sr-only">{tx(locale, "공지 검색", "Search notices")}</span><Search size={17} /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={tx(locale, "제목·작성자 검색", "Search title or author")} /></label></div><div className="notice-table"><div className="notice-table-head"><span>{tx(locale, "구분", "Category")}</span><span>{tx(locale, "제목", "Title")}</span><span>{tx(locale, "게시일", "Date")}</span></div>{visibleResults.map((notice) => <NoticeRow key={notice.id} notice={notice} locale={locale} />)}</div>{!results.length && <EmptyState locale={locale} />}{totalPages > 1 && <nav className="pagination" aria-label={tx(locale, "페이지", "Pagination")}><button type="button" disabled={page === 1} onClick={() => setPage((current) => current - 1)} aria-label={tx(locale, "이전 페이지", "Previous page")}><ChevronLeft size={18} /></button>{Array.from({ length: totalPages }, (_, index) => index + 1).map((number) => <button type="button" key={number} aria-current={page === number ? "page" : undefined} onClick={() => setPage(number)}>{number}</button>)}<button type="button" disabled={page === totalPages} onClick={() => setPage((current) => current + 1)} aria-label={tx(locale, "다음 페이지", "Next page")}><ChevronRight size={18} /></button></nav>}</div></section></>;
}

function NoticeDetail({ locale, notice }: { locale: Locale; notice: Notice }) {
  const orderedNotices = [...notices].sort((a, b) => Number(b.isPinned) - Number(a.isPinned) || b.publishedAt.localeCompare(a.publishedAt));
  const index = orderedNotices.findIndex((item) => item.id === notice.id);
  const previous = orderedNotices[index - 1];
  const next = orderedNotices[index + 1];
  return <><PageHeader eyebrow="NOTICE" title={t(notice.title, locale)} description={`${tx(locale, notice.category, "Notice")} · ${notice.publishedAt.replaceAll("-", ".")}`} /><section className="section content-section"><article className="container article-detail"><div className="article-meta"><span>{notice.isPinned ? tx(locale, "고정 공지", "Pinned notice") : tx(locale, notice.category, "Notice")}</span><span>{t(notice.author, locale)}</span><time dateTime={notice.publishedAt}>{notice.publishedAt.replaceAll("-", ".")}</time><span>{notice.hasAttachment ? tx(locale, "첨부파일 있음", "Attachment available") : tx(locale, "첨부파일 없음", "No attachment")}</span></div><a className="button outline official-post-link" href={notice.sourceUrl} target="_blank" rel="noopener noreferrer" aria-label={tx(locale, `${t(notice.title, locale)} 공식 게시글 보기, 새 탭에서 열림`, `View original post: ${t(notice.title, locale)}, opens in a new tab`)}>{tx(locale, "공식 게시글 보기", "View Original Post")}<ExternalLink size={16} aria-hidden="true" /></a><div className="article-navigation">{previous ? <Link href={hrefFor(locale, `/news/notices/${previous.slug}`)}><ChevronLeft size={18} /><span><small>{tx(locale, "이전글", "Previous")}</small>{t(previous.title, locale)}</span></Link> : <span />}{next ? <Link href={hrefFor(locale, `/news/notices/${next.slug}`)}><span><small>{tx(locale, "다음글", "Next")}</small>{t(next.title, locale)}</span><ChevronRight size={18} /></Link> : <span />}</div><Link className="button outline article-list-button" href={hrefFor(locale, "/news/notices")}><ArrowLeft size={17} />{tx(locale, "목록으로", "Back to list")}</Link></article></section></>;
}
