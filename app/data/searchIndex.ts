...
const documents: SearchDocument[] = [
  ...
  ...notices.map((notice) => ({
    ...
    title: notice.title,
    description: notice.body ?? { ko: "", en: "" },
    path: `/news/notices/${notice.slug}`,
    searchableText: join([localized(notice.title), localized(notice.body), localized(notice.author), notice.category, notice.audience]),
  })),
  ...[...newsPosts, ...seminarPosts, ...eventPosts].map((post) => ({
      id: `editorial-${post.id}`,
      kind: "notice-news" as const,
      title: post.title,
      description: post.summary ?? { ko: "", en: "" },
